# 社區感測器資料上傳平台 - 完整操作手冊

## 📋 目錄

- [專案概述](#專案概述)
- [系統架構](#系統架構)
- [功能特色](#功能特色)
- [快速開始](#快速開始)
- [詳細安裝指南](#詳細安裝指南)
- [系統啟動](#系統啟動)
- [API 文檔](#api-文檔)
- [使用範例](#使用範例)
- [效能優化](#效能優化)
- [故障排除](#故障排除)
- [開發指南](#開發指南)

---

## 專案概述

這是一個完整的社區感測器資料上傳平台，包含後端引擎、前端儀表板和數據模擬器。系統能夠智能處理感測器數據的上傳優先級，並提供即時監控和報表功能。

### 技術棧

- **後端**: Node.js + Express
- **前端**: React + Vite + ECharts
- **模擬器**: Python
- **效能優化**: 快取機制、非同步處理、演算法優化

---

## 系統架構

```
┌─────────────┐         ┌─────────────┐         ┌─────────────┐
│   模擬器    │ ──────> │   後端 API  │ <────── │   前端      │
│  (Python)   │  POST   │  (Node.js)  │  GET    │  (React)    │
└─────────────┘         └─────────────┘         └─────────────┘
                              │
                              ▼
                    ┌─────────────────┐
                    │  優先級引擎     │
                    │  上傳調度器     │
                    │  快取服務       │
                    └─────────────────┘
```

### 核心組件

1. **後端引擎** (`server.js`)
   - RESTful API 服務
   - 優先級判定引擎
   - 上傳調度器
   - 快取機制
   - 效能監控

2. **前端儀表板** (`frontend/`)
   - React 應用程式
   - 即時數據顯示
   - 圖表視覺化
   - 數據上傳介面

3. **數據模擬器** (`tests/simulator_backend.py`)
   - 自動生成感測器數據
   - 支援多種測試場景
   - 持續運行模式

---

## 功能特色

### ✅ 第一階段：基礎架構與 API 定義

- Node.js + Express 伺服器環境
- RESTful API 設計
  - **輸入端**：接收感測器數據（POST /api/sensors/data）
  - **輸出端**：提供數據查詢與報表（GET /api/sensors/data, GET /api/reports/*）

### ✅ 第二階段：優先級判定引擎

- **優先級計算演算法**
  - 根據三個權重計算優先級分數（0-10）：
    - 資料重要性 (Data Importance) - 權重 0.5
    - 節點狀態 (Battery) - 權重 0.3
    - 網路狀況 (Network Status) - 權重 0.2
  
- **上傳調度器 (Upload Scheduler)**
  - **Preemptive (搶佔式)**：緊急資料插隊機制，確保關鍵資料優先送達
  - **Batch (批次)**：非緊急資料打包一起送，節省資源

### ✅ 第三階段：前端、模擬器、後端連接

- 統一 JSON 格式規範
- 模擬器自動生成數據
- 前端即時監控儀表板
- 完整數據流程

### ✅ 第四階段：效能優化

- **快取機制**：減少資料庫讀取次數，提升回應速度
- **非同步處理**：優先級計算不阻塞主線程，提升吞吐量
- **演算法優化**：降低運算成本，提升計算效率
- **效能監控**：即時監控系統效能指標

---

## 快速開始

### 前置需求

- **Node.js** >= 14.0.0
- **Python** >= 3.7
- **npm** 或 **yarn**

### 1. 安裝依賴

```bash
# 後端依賴
npm install

# 前端依賴
cd frontend
npm install
cd ..
```

### 2. 啟動系統

**終端 1：啟動後端**
```bash
npm start
```

**終端 2：啟動前端**
```bash
cd frontend
npm run dev
```

**終端 3：運行模擬器（可選）**
```bash
python tests/simulator_backend.py --continuous
```

### 3. 訪問系統

- **前端儀表板**: http://localhost:5173
- **後端 API**: http://localhost:3000
- **API 文檔**: http://localhost:3000/
- **健康檢查**: http://localhost:3000/health

---

## 詳細安裝指南

### 環境檢查

```bash
# 檢查 Node.js 版本
node --version

# 檢查 Python 版本
python --version

# 檢查 npm 版本
npm --version
```

### 完整安裝步驟

#### 步驟 1：克隆或下載專案

```bash
cd "c:\CSIE\computer network\final_project"
```

#### 步驟 2：安裝後端依賴

```bash
npm install
```

這會安裝以下套件：
- `express` - Web 框架
- `cors` - 跨域支援
- `body-parser` - 請求解析
- `dotenv` - 環境變數

#### 步驟 3：安裝前端依賴

```bash
cd frontend
npm install
cd ..
```

這會安裝以下套件：
- `react` - UI 框架
- `react-dom` - React DOM 渲染
- `echarts` - 圖表庫
- `vite` - 構建工具
- `tailwindcss` - CSS 框架

#### 步驟 4：安裝 Python 依賴（模擬器）

```bash
pip install requests
```

---

## 系統啟動

### 啟動方式

#### 方式一：標準啟動（推薦）

**1. 啟動後端伺服器**

```bash
# 開發模式（自動重啟）
npm run dev

# 生產模式
npm start
```

**預期輸出：**
```
🚀 伺服器運行於 http://localhost:3000
📊 API 文檔: http://localhost:3000/
❤️  健康檢查: http://localhost:3000/health
```

**2. 啟動前端開發伺服器**

```bash
cd frontend
npm run dev
```

**預期輸出：**
```
  VITE v5.x.x  ready in xxx ms

  ➜  Local:   http://localhost:5173/
  ➜  Network: use --host to expose
```

**3. 運行模擬器（可選）**

```bash
# 持續運行模式（自動生成數據）
python tests/simulator_backend.py --continuous

# 運行指定時間（例如 60 秒）
python tests/simulator_backend.py --continuous --duration 60

# 固定數量模式
python tests/simulator_backend.py --sensors 5 --messages 10
```

#### 方式二：一鍵啟動腳本（Windows）

創建 `start-all.bat`：

```batch
@echo off
start "後端伺服器" cmd /k "npm start"
timeout /t 3
start "前端開發伺服器" cmd /k "cd frontend && npm run dev"
timeout /t 3
start "模擬器" cmd /k "python tests/simulator_backend.py --continuous"
```

### 驗證系統運行

**1. 檢查後端健康狀態**

```bash
curl http://localhost:3000/health
```

**預期回應：**
```json
{
  "status": "healthy",
  "timestamp": "2024-01-15T10:30:00.000Z",
  "uptime": 123.45
}
```

**2. 檢查前端連接**

打開瀏覽器訪問 `http://localhost:5173`，右上角應顯示「✅ 已連接」

**3. 檢查模擬器**

模擬器控制台應顯示：
```
🚀 開始模擬器測試
📡 後端 URL: http://localhost:3000
🔄 模式: 持續運行（自動生成數據）
```

---

## API 文檔

### 基礎資訊

- **Base URL**: `http://localhost:3000`
- **Content-Type**: `application/json`

### 感測器數據 API

#### 上傳感測器數據

```
POST /api/sensors/data
```

**請求體範例：**
```json
{
  "nodeId": "S-001",
  "dataImportance": 8.5,
  "battery": 75.0,
  "timestamp": "2024-01-15T10:30:00.000Z",
  "networkStatus": "good",
  "sensorType": "temperature",
  "value": 25.5,
  "unit": "celsius",
  "periodic": {
    "temperature": 25.5,
    "humidity": 65.2,
    "rain_prob": 0.3,
    "wind_speed": 5.2,
    "wind_dir": "E",
    "pressure": 1013.5,
    "AQI": 45,
    "noise": 55,
    "traffic": "MEDIUM",
    "notice": "none"
  },
  "emergency": null,
  "metadata": {
    "personal_id": "uuid-string",
    "scenario_id": "baseline",
    "send_unix": 1705312200.123
  }
}
```

**必填欄位：**
- `nodeId`: 節點 ID（字串）
- `dataImportance`: 資料重要性（0-10 的數字）
- `battery`: 電量（0-100 的數字）

**回應範例：**
```json
{
  "success": true,
  "message": "感測器數據上傳成功",
  "data": {
    "id": "sensor-1",
    "nodeId": "S-001",
    "priority": {
      "priorityScore": 7.85,
      "priorityLevel": "high",
      "breakdown": { ... }
    },
    "scheduleResult": {
      "scheduled": true,
      "queueType": "preemptive",
      "priority": "high"
    }
  }
}
```

#### 獲取所有感測器數據

```
GET /api/sensors/data
```

**查詢參數：**
- `limit`: 限制返回數量（數字）
- `offset`: 偏移量（數字）
- `nodeId`: 篩選特定節點（字串）
- `sensorType`: 篩選特定感測器類型（字串）
- `priorityLevel`: 篩選優先級等級（critical, high, medium, low）
- `minPriorityScore`: 最小優先級分數（0-10）
- `sortBy`: 排序方式（priority, timestamp）

**範例：**
```bash
# 按優先級排序，限制 10 筆
GET /api/sensors/data?sortBy=priority&limit=10

# 篩選高優先級數據
GET /api/sensors/data?priorityLevel=high

# 分頁查詢
GET /api/sensors/data?limit=20&offset=0
```

#### 獲取特定感測器數據

```
GET /api/sensors/data/:id
```

#### 獲取節點數據

```
GET /api/sensors/node/:nodeId
```

#### 獲取優先級統計

```
GET /api/sensors/priority/stats
```

### 報表 API

#### 獲取數據摘要

```
GET /api/reports/summary
```

**回應範例：**
```json
{
  "success": true,
  "data": {
    "totalRecords": 100,
    "uniqueNodes": 5,
    "nodeList": ["S-001", "S-002"],
    "sensorTypes": {
      "temperature": 20,
      "humidity": 20
    },
    "averageBattery": 72.5,
    "averageImportance": 6.8,
    "networkStatus": {
      "good": 80,
      "fair": 15
    },
    "latestTimestamp": "2024-01-15T10:30:00.000Z"
  }
}
```

#### 獲取統計數據

```
GET /api/reports/statistics?timeRange=all
```

**時間範圍選項：**
- `today`: 今天
- `week`: 最近一週
- `month`: 最近一個月
- `all`: 全部（預設）

### 調度器 API

#### 獲取上傳佇列狀態

```
GET /api/scheduler/queue
```

#### 手動觸發緊急佇列處理

```
POST /api/scheduler/process-critical
```

#### 手動觸發批次佇列處理

```
POST /api/scheduler/process-batch
```

#### 清空所有佇列

```
DELETE /api/scheduler/queue
```

### 效能監控 API

#### 獲取效能統計

```
GET /api/performance/stats
```

**回應範例：**
```json
{
  "success": true,
  "performance": {
    "totalRequests": 1000,
    "averageResponseTime": 45.2,
    "p50": 38.5,
    "p95": 89.3,
    "p99": 125.7
  },
  "cache": {
    "hits": 750,
    "misses": 250,
    "hitRate": "75.00%",
    "size": 50
  },
  "asyncQueue": {
    "queueLength": 0,
    "isProcessing": false
  }
}
```

#### 重置效能統計

```
POST /api/performance/reset
```

---

## 使用範例

### 範例 1：上傳感測器數據

```bash
curl -X POST http://localhost:3000/api/sensors/data \
  -H "Content-Type: application/json" \
  -d '{
    "nodeId": "S-001",
    "dataImportance": 8.5,
    "battery": 75.0,
    "networkStatus": "good",
    "sensorType": "temperature",
    "value": 25.5,
    "periodic": {
      "temperature": 25.5,
      "humidity": 65.2,
      "AQI": 45
    }
  }'
```

### 範例 2：查詢高優先級數據

```bash
curl "http://localhost:3000/api/sensors/data?priorityLevel=high&sortBy=priority&limit=10"
```

### 範例 3：獲取系統統計

```bash
# 獲取摘要
curl http://localhost:3000/api/reports/summary

# 獲取效能統計
curl http://localhost:3000/api/performance/stats
```

### 範例 4：使用模擬器

```bash
# 持續運行模式（推薦）
python tests/simulator_backend.py --continuous

# 自訂參數
python tests/simulator_backend.py --continuous \
  --sensors 10 \
  --interval 1.0 \
  --scenario baseline \
  --duration 300
```

---

## 效能優化

### 優化功能

系統已實作以下效能優化：

1. **快取機制**
   - 記憶體快取減少重複查詢
   - 自動過期機制（TTL）
   - 快取命中率監控

2. **非同步處理**
   - 優先級計算不阻塞主線程
   - 任務佇列管理
   - 批量並行處理

3. **演算法優化**
   - 減少函數調用開銷
   - 直接計算優化
   - 單次遍歷統計

### 效能測試

```bash
# 執行效能測試
npm run test:performance

# 查看效能統計
curl http://localhost:3000/api/performance/stats
```

### 預期效果

- **回應時間降低 30% 以上**
- **快取命中率 70% 以上**
- **吞吐量提升**
- **CPU 使用率降低**

詳細說明請查看 [效能優化文檔](docs/performance-optimization.md)

---

## 故障排除

### 常見問題

#### 1. 後端無法啟動

**問題：** `Error: listen EADDRINUSE: address already in use`

**解決：**
```bash
# Windows: 查找占用端口的進程
netstat -ano | findstr :3000

# 終止進程（替換 PID）
taskkill /PID <PID> /F

# 或修改端口
# 在 .env 文件中設定 PORT=3001
```

#### 2. 前端無法連接後端

**問題：** `ERR_CONNECTION_REFUSED`

**解決：**
1. 確認後端已啟動：訪問 `http://localhost:3000/health`
2. 檢查 CORS 設定
3. 確認前端 API_BASE_URL 設定正確

#### 3. 模擬器連接失敗

**問題：** `Connection refused` 或 `無法連接`

**解決：**
1. 確認後端伺服器已啟動
2. 檢查後端 URL 是否正確
3. 檢查防火牆設定
4. 確認 Python requests 套件已安裝

#### 4. 數據格式錯誤

**問題：** `400 Bad Request` 或驗證錯誤

**解決：**
1. 確認必填欄位：`nodeId`, `dataImportance`, `battery`
2. 確認數據範圍：
   - `dataImportance`: 0-10
   - `battery`: 0-100
3. 檢查 JSON 格式是否正確

#### 5. 快取問題

**問題：** 數據未更新

**解決：**
```bash
# 查看快取統計
curl http://localhost:3000/api/performance/stats

# 快取會自動過期，或等待 30 秒後重試
```

### 日誌檢查

**後端日誌：**
- 查看控制台輸出
- 檢查錯誤訊息
- 確認請求日誌

**前端日誌：**
- 打開瀏覽器開發者工具（F12）
- 查看 Console 標籤
- 檢查 Network 標籤的請求狀態

---

## 開發指南

### 專案結構

```
.
├── server.js                 # 主伺服器檔案
├── controllers/              # 控制器
│   ├── sensorController.js   # 感測器控制器
│   ├── reportController.js   # 報表控制器
│   └── schedulerController.js # 調度器控制器
├── services/                 # 業務邏輯服務
│   ├── sensorService.js      # 感測器服務
│   ├── reportService.js      # 報表服務
│   ├── priorityEngine.js     # 優先級判定引擎
│   ├── uploadScheduler.js    # 上傳調度器
│   ├── cacheService.js       # 快取服務
│   └── asyncProcessor.js     # 非同步處理服務
├── routes/                   # 路由定義
│   ├── sensorRoutes.js       # 感測器路由
│   ├── reportRoutes.js       # 報表路由
│   └── schedulerRoutes.js    # 調度器路由
├── middleware/               # 中間件
│   └── performanceMonitor.js # 效能監控
├── frontend/                 # 前端應用
│   ├── src/
│   │   ├── App.jsx           # 主應用程式
│   │   └── main.jsx          # 入口點
│   └── package.json
├── tests/                    # 測試檔案
│   ├── api.test.js           # API 測試
│   ├── performance-test.js  # 效能測試
│   └── simulator_backend.py  # 數據模擬器
└── docs/                     # 文檔
    ├── api-specification.md  # API 規範
    ├── connection-guide.md   # 連接指南
    └── performance-optimization.md # 效能優化
```

### 開發命令

```bash
# 後端開發（自動重啟）
npm run dev

# 前端開發（熱重載）
cd frontend && npm run dev

# 執行測試
npm test

# 執行效能測試
npm run test:performance

# 構建前端
cd frontend && npm run build
```

### 程式碼規範

- 使用 ES6+ 語法
- 遵循 RESTful API 設計原則
- 統一錯誤處理格式
- 添加適當的註釋

### 測試

```bash
# API 功能測試
npm test

# 效能測試
npm run test:performance

# 連接測試
node test-connection.js
```

---

## 進階功能

### 環境變數配置

創建 `.env` 文件：

```env
# 伺服器設定
PORT=3000
NODE_ENV=development

# 前端 API 基礎 URL（前端使用）
VITE_API_BASE_URL=http://localhost:3000
```

### 自訂配置

**修改後端端口：**
```env
PORT=3001
```

**修改前端端口：**
編輯 `frontend/vite.config.js`：
```javascript
server: {
  port: 5174
}
```

### 生產環境部署

**1. 構建前端**
```bash
cd frontend
npm run build
```

**2. 使用 PM2 管理後端**
```bash
npm install -g pm2
pm2 start server.js --name sensor-platform
```

**3. 使用 Nginx 反向代理**
```nginx
server {
    listen 80;
    server_name your-domain.com;

    location / {
        proxy_pass http://localhost:3000;
    }
}
```

---

## 相關文檔

- [API 規範](docs/api-specification.md) - 詳細的 API 格式規範
- [連接指南](docs/connection-guide.md) - 前端、模擬器、後端連接說明
- [模擬器使用指南](docs/simulator-guide.md) - 模擬器詳細使用說明
- [效能優化文檔](docs/performance-optimization.md) - 效能優化詳細說明

---

## 授權

ISC

---

## 支援與回饋

如有問題或建議，請查看相關文檔或檢查日誌輸出。

**最後更新：** 2024-01-15
