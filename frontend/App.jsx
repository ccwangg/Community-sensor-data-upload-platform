    ...
    useEffect(() => {
        if (chartInstance.current && options && Object.keys(options).length > 0) {
            try {
                ...
                console.error("ECharts setOption 失敗:", error, "chartId:", chartId, "options:", options);
            }
        } else if (chartInstance.current && (!options || Object.keys(options).length === 0)) {
            // 如果沒有選項，設置一個空圖表
            try {
                ...
                console.error("ECharts 設置空圖表失敗:", error);
            }
        }
    }, [options, chartId]);
...
const createTempHumidityOptions = (readings) => {
    const displayReadings = readings.length > 0 ? readings.slice(-10) : [];
    const timestamps = displayReadings.length > 0
        ? displayReadings.map(r => 
            r.timestamp ? new Date(r.timestamp).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' }) : '載入中...'
        )
        : ['暫無數據'];
    const tempValues = displayReadings.length > 0
        ? displayReadings.map(r => r.periodic?.temperature || r.value || 0)
        : [0];
    const humidityValues = displayReadings.length > 0
        ? displayReadings.map(r => r.periodic?.humidity || 0)
        : [0];
...
const createPressureTrendOptions = (readings) => {
    const displayReadings = readings.length > 0 ? readings.slice(-15) : [];
    const timestamps = displayReadings.length > 0 
        ? displayReadings.map(r => 
            r.timestamp ? new Date(r.timestamp).toLocaleTimeString('zh-TW', { hour: '2-digit', minute: '2-digit' }) : '載入中...'
        )
        : ['暫無數據'];
    const pressureValues = displayReadings.length > 0 
        ? displayReadings.map(r => r.periodic?.pressure || 0)
        : [0];
...
const createNoiseTrendOptions = (readings) => {
    ...
    const timestamps = displayReadings.length > 0
        ...
        : ['暫無數據'];
    const noiseValues = displayReadings.length > 0
        ? displayReadings.map(r => r.periodic?.noise || 0)
        : [0];
...
const createWindOptions = (readings) => {
    ...
    const timestamps = displayReadings.length > 0
        ...
        : ['暫無數據'];
    const windSpeedValues = displayReadings.length > 0
        ? displayReadings.map(r => r.periodic?.wind_speed || 0)
        : [0];
    const windDirectionData = displayReadings.length > 0
...
// --- 圖表容器元件 ---
const ChartContainer = ({ options, id }) => {
    // 確保有有效的選項（即使沒有數據也要顯示圖表框架）
    ...
    const chartRef = useEcharts(validOptions, id);
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
            <div className="bg-gray-800 rounded-xl shadow-2xl max-w-2xl w-full max-h-[90vh] overflow-y-auto">
                <div className="p-6 border-b border-gray-700">
                    <h2 className="text-2xl font-bold text-indigo-400">模擬上傳感測器數據</h2>
                    <p className="text-gray-400 text-sm mt-1">填寫感測器數據並上傳到後端 API</p>
                </div>
                <form onSubmit={handleSubmit} className="p-6 space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">節點 ID</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">資料重要性 (0-10)</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">電量 (0-100)</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">溫度 (°C)</label>
                        ...
                        <div>
                            <label className="block text-sm font-medium text-gray-300 mb-1">濕度 (%)</label>
        ...
        const loadData = async () => {
            try {
                ...
                if (response.ok) {
                    ...
                    setLoadingError(`載入數據失敗: ${response.status}`);
                }
            } catch (error) {
                console.error('載入數據失敗:', error);
                setLoadingError(`載入數據錯誤: ${error.message}`);
            }
        };
        ...
        <div className="min-h-screen bg-gray-900 text-gray-100 p-4 sm:p-8 font-sans">
            <header className="mb-6">
                ...
                <p className="text-gray-400">
                    即時環境數據監測 ({isConnected ? '✅ 已連接後端' : '❌ 未連接後端'})
                </p>
            </header>
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-5 gap-4 mb-8">
                ...
                <div className="bg-gray-800 p-5 rounded-xl shadow-xl border-t-4 border-purple-500">
                    <h3 className="text-sm font-medium text-gray-400 uppercase">氣壓</h3>
                ...
                <div className="bg-gray-800 p-5 rounded-xl shadow-xl border-t-4 border-pink-500">
                    <h3 className="text-sm font-medium text-gray-400 uppercase">優先級</h3>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
                ...
                <div className="bg-gray-800 p-6 rounded-xl shadow-xl">
                    <h2 className="text-xl font-bold text-indigo-400 mb-4">AQI 趨勢</h2>
                    <ChartContainer options={pm25Options} id="pm25-chart" />
                </div>
                
                {/* 溫度與濕度 */}
                <div className="bg-gray-800 p-6 rounded-xl shadow-xl">
                    <h2 className="text-xl font-bold text-indigo-400 mb-4">溫度與濕度</h2>
                    <ChartContainer options={tempHumidityOptions} id="temp-humidity-chart" />
                </div>
                
                {/* 氣壓趨勢 */}
                <div className="bg-gray-800 p-6 rounded-xl shadow-xl">
                    <h2 className="text-xl font-bold text-indigo-400 mb-4">氣壓趨勢</h2>
                    <ChartContainer options={pressureOptions} id="pressure-chart" />
                </div>
                
                {/* 噪音趨勢 */}
                <div className="bg-gray-800 p-6 rounded-xl shadow-xl">
                    <h2 className="text-xl font-bold text-indigo-400 mb-4">噪音趨勢</h2>
                    <ChartContainer options={noiseOptions} id="noise-chart" />
                </div>
                
                {/* 風速與風向 */}
                <div className="bg-gray-800 p-6 rounded-xl shadow-xl lg:col-span-2">
                    <h2 className="text-xl font-bold text-indigo-400 mb-4">風速與風向</h2>
                    <ChartContainer options={windOptions} id="wind-chart" />
                </div>
            </div>

            {/* 數據列表 */}
            <div className="bg-gray-800 p-6 rounded-xl shadow-xl mb-8">
                <h2 className="text-xl font-bold text-indigo-400 mb-4">最新數據列表</h2>
                <div className="overflow-x-auto">
                    <table className="w-full text-sm">
                        <thead>
                            <tr className="border-b border-gray-700">
                                <th className="text-left p-2">節點 ID</th>
                                <th className="text-left p-2">優先級</th>
                                <th className="text-left p-2">溫度</th>
                                <th className="text-left p-2">濕度</th>
                                <th className="text-left p-2">電量</th>
                        ...
                        <tbody>
                            {readings.slice(0, 10).map((reading, index) => (
                                <tr key={reading.id || index} className="border-b border-gray-700">
                                    ...
                                    <td className="p-2">{reading.periodic?.temperature || reading.value || 'N/A'}°C</td>
            ...
            <footer className="mt-12 text-center text-gray-600 border-t border-gray-800 pt-6">
                <p>社區感測器資料上傳平台 - 連接後端 API (優先級引擎已啟用)</p>
            </footer>
